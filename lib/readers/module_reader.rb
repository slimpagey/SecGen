  ...
  def self.read_modules(module_type, modules_dir, schema_file, require_puppet)
    ...
    Dir.glob("#{modules_dir}**/**/secgen_metadata.xml").each do |file|
      ...
      doc.xpath("/#{module_type}/default_input").each do |inputs_doc|
        Print.std "inputs_doc = #{inputs_doc.to_s}"
        inputs_doc.xpath('descendant::vulnerability | descendant::service | descendant::utility | descendant::network | descendant::base | descendant::encoder | descendant::generator').each do |module_node|
          Print.err "module_node = #{module_node.to_s}"
          ...
          Print.debug "module_selector.unique_id = #{module_selector.unique_id}"
          ...
          module_node.xpath('parent::input').each do |input|
            ...
            input.xpath('..').each do |input_parent|
              ...
              Print.debug "+module_selector.write_output_variable: #{module_selector.write_output_variable}"
              Print.debug "+module_selector.write_to_module_with_id = #{module_selector.write_to_module_with_id}"
            end
          end
          # TODO FIXME GENERATORS NOT COMING THROUGH????????????????????????
          Print.err "module_node.xpath('parent::default_input'): #{module_node.xpath('parent::default_input')}"
          ...
          Print.debug "Adding default module: #{into} #{module_selector.inspect}"

          # SystemReader.insert_selector_into_list(module_selector, (new_module.default_inputs_selectors["#{into}"] ||= []))
        ...
        inputs_doc.xpath('value').each do |input_value|
          ...
          value = input_value.text
          Print.verbose "  -- d literal value: #{variable} = #{value}"
    ...
    Print.debug "modules: #{modules.inspect}"
