...
class System
  ...
  def select_modules(module_type, required_attributes, available_modules, previously_selected_modules, unique_id, write_outputs_to, write_to_module_with_id, received_inputs, default_inputs_literals)
    ...
    if search_list.length == 0
      ...
      Print.debug "adding default modules: #{default_modules_to_add.inspect} "

      # feed in the input from any previous module's output
      Print.debug "aaaaaaaaaaaaaaaaaaaaaaaaa\nfeeding into selected.unique_id: #{selected.unique_id} \n"
      (previously_selected_modules + default_modules_to_add).each do |previous_module|
        ...
        Print.debug "previous_module.unique_id: #{previous_module.unique_id} \nprevious_module.module_path: #{previous_module.module_path} \nprevious_module.write_to_module_with_id #{previous_module.write_to_module_with_id} \nprevious_module.write_output_variable: #{previous_module.write_output_variable}"
        if previous_module.write_to_module_with_id == selected.unique_id && previous_module.write_output_variable
          Print.debug "Writing to #{unique_id}.#{previous_module.write_output_variable} with = #{previous_module.output}"
          (selected.received_inputs[previous_module.write_output_variable] ||=[]).push(*previous_module.output)
          Print.debug "output = #{selected.received_inputs[previous_module.write_output_variable]}"
        end
      end
    ...
    Print.err "selected_modules: #{selected_modules.inspect}"
  ...
  def select_default_modules(selected, available_modules, previously_selected_modules)
    ...
    Print.err default_keys

    # check whether each defaults should be used
    # for each variable with a default
    Print.verbose "defaults: #{default_keys.inspect}"
    default_keys.each do |default_key|
      Print.verbose "default: #{default_key}"

      Print.debug "Received input: #{selected.received_inputs[default_key]}"
      ...
      unless selected.received_inputs.has_key? default_key
        ...
        if selected.default_inputs_literals.has_key? default_key
          Print.debug "Using literal input #{selected.default_inputs_literals[default_key]}"
          (selected.received_inputs[default_key] ||=[]).concat selected.default_inputs_literals[default_key]
        end
        Print.debug "Default literals: #{selected.default_inputs_literals[default_key]}"

        # apply modules
        if selected.default_inputs_selectors.has_key? default_key
          Print.debug "Using module input #{selected.default_inputs_selectors[default_key].inspect}"
          ...
          default_module_selectors_to_add.each do |module_to_add|
            ...
            # Print.err "default_modules_to_add = #{default_modules_to_add.inspect}"

          end

        end

        # # feed forward the input from any default module's output
        # (default_modules_to_add).each do |previous_module|
        #
        #   # if previous_module.write_to_module_with_id == unique_id && previous_module.write_output_variable && !variables_received_from_scenario.include?(previous_module.write_output_variable)
        #   if previous_module.write_to_module_with_id == selected.unique_id && previous_module.write_output_variable
        #     Print.debug "Writing to #{selected.unique_id}.#{previous_module.write_output_variable} with = #{previous_module.output}"
        #     (selected.received_inputs[previous_module.write_output_variable] ||=[]).push(*previous_module.output)
        #     Print.debug "output = #{selected.received_inputs[previous_module.write_output_variable]}"
        #   end
        # end
